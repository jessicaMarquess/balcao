...
import type { Produto, NovoProduto } from '../../renderer/src/types'

export function listarProdutos(): Produto[] {
  return getDb().prepare('SELECT * FROM produtos WHERE ativo = 1 ORDER BY nome ASC').all() as Produto[]
}
...
export function atualizarProduto(id: number, dados: Partial<NovoProduto>): Produto {
  ...
  if (dados.preco !== undefined) { campos.push('preco = ?'); valores.push(dados.preco) }
  if (dados.estoque !== undefined) { campos.push('estoque = ?'); valores.push(dados.estoque) }
